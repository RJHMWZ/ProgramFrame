...
public class PoolMgr3 : Singleton2<PoolMgr3>
{
    private PoolMgr3()
    {
    ...
    private readonly Dictionary<string, PoolData2> poolDic = new();   // 柜子：Key 是对象类型，Value 是对应的抽屉
    ...
    public GameObject GetObj(string name, int maxNum = 10)
    {
       if (string.IsNullOrEmpty(name))
        ...
        // 情况一：还没有这种对象对应的抽屉
        if (!poolDic.TryGetValue(name, out PoolData2 PoolData2))
        {
            ...
            PoolData2 = new PoolData2(poolRoot, name, newObj);
            poolDic.Add(name, PoolData2);
            return newObj;
        }

        // 情况二：抽屉中存在空闲对象，直接复用
        if (PoolData2.Count > 0)
        {
            return PoolData2.Pop();
        }

        // 情况三：没有空闲对象，但还没有达到数量上限
        if (PoolData2.UsedCount < maxNum)
        {
            GameObject newObj = CreateObj(name);
    ...
    public void PushObj(GameObject obj)
    {
       if (obj == null)
        ...
        if (!poolDic.TryGetValue(obj.name, out PoolData2 PoolData2))
        {
            PoolData2 = new PoolData2(poolRoot, obj.name, obj);
            poolDic.Add(obj.name, PoolData2);
        }
